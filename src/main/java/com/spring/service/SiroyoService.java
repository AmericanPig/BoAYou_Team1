...
public interface SiroyoService {
	...
	int pushCommunitySiroyo(@Param("community_no") int community_no, @Param("user_id") String user_id);
	
	//커뮤니티 특정 글 페이지 특정 댓글의 좋아요 버튼 toggle
	int pushCommentSiroyo(@Param("comment_no") int comment_no, @Param("user_id") String user_id);
