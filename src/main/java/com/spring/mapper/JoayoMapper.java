...
public interface JoayoMapper {
	...
	int selectCommunityJoayoById(@Param("community_no") int community_no, @Param("user_id") String user_id);
	
	//커뮤니티 글에 특정 user_id가 조아요 insert
	int insertCommunityJoayoById(@Param("community_no") int community_no, @Param("user_id") String user_id);
	
	//커뮤니티 글에 특정 user_id가 조아요 delete
	int deleteCommunityJoayoById(@Param("community_no") int community_no, @Param("user_id") String user_id);
	
	//커뮤니티 페이지에서 해당 글의 누적조아요 개수 select
	int selectCommunityJoayoCnt(int community_no);
	
	//특정 comment의 특정 user_id 조아요 있는지 확인 select
	int selectCommentJoayoById(@Param("comment_no")int comment_no, @Param("user_id") String user_id);
	
	//특정 comment의 특정 user_id가 조아요 insert
	int insertCommentJoayoById(@Param("comment_no")int comment_no, @Param("user_id") String user_id);
	
	//특정 comment에 특정 user_id 조아요 delete
	int deleteCommentJoayoById(@Param("comment_no")int comment_no, @Param("user_id") String user_id);
