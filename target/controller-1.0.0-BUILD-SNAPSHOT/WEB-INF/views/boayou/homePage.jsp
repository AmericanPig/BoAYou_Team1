...
$(document).ready(function() {
    $("#searchBox2")
        .keyup(function() { // Keyup 이벤트가 발생할 때마다
...
function performSearch(keyword) {
    $.ajax({
        url: "/controller/boayou/search", // 검색 URL로 전송
        dataType: "json",
        data: { "user_id": keyword }, // user_id 파라미터를 사용해 keyword를 가져옵니다.
        success: function(data) { // Ajax 요청이 성공하면
            displaySearchResults(data); // 결과를 표시합니다.
        }
    });
}

var searchTimeout; // 검색 대기 시간 설정

$("#searchBox2").keyup(function() {
    clearTimeout(searchTimeout); // 이전 검색 대기 시간을 초기화
    var keyword = $(this).val();
    searchTimeout = setTimeout(function() {
        performSearch(keyword); // 새로운 검색 대기 시간 설정
    }, 100); // 300ms 후 검색 시작
});

function displaySearchResults(data) {
    var searchResults2 = $("#searchResults2");
    searchResults2.empty(); // 기존 검색 결과를 비웁니다.
